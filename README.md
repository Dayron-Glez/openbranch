...
Playground challenges live in `content/playground/` as MDX files, also bilingual (`.es.mdx` / `.en.mdx`). In addition to the guide fields, each challenge declares its `category` (bug-fix, code-review, testing, git, documentation), `difficulty`, `estimated_minutes`, the `validation` style shown to the reader, and the `skills` it exercises. The brief is only half of a challenge — the workspace and its grading logic are TypeScript under `features/playground/challenges/<category>/`, selected by `category` and wired up through that engine's registry. See [CONTRIBUTING.md](./CONTRIBUTING.md#adding-a-playground-challenge).
