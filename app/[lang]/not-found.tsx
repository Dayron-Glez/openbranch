...
export default async function NotFound({
  ...
  return (
    <>
      ...
      <main className="relative z-1 flex min-h-[calc(100dvh-260px)] items-center justify-center px-8 py-16 max-[520px]:px-5 max-[520px]:py-11">
        <div className="flex max-w-[600px] flex-col items-center text-center">
          <div
            className="border-accent-ring after:border-line relative mb-[34px] grid size-[104px] shrink-0 place-items-center rounded-(--r-full) border after:absolute after:-inset-[7px] after:rounded-(--r-full) after:border after:content-['']"
            style={{
              background:
                "radial-gradient(ellipse 100% 100% at 50% 20%, var(--color-accent-soft), transparent 75%), var(--color-bg-elev)",
            }}
          >
            <LogoMark size={56} broken />
          </div>

          <p className="text-fg-muted mb-4.5 inline-flex items-center gap-2.5 font-mono text-[11px] tracking-[0.08em] uppercase">
          ...
          <h1 className="text-fg m-0 mb-4.5 max-w-[16ch] text-[42px] leading-[1.06] font-normal tracking-[-0.03em] text-balance max-[520px]:text-[32px]">
            {dict.headingLead} <span className="text-fg-2 font-light">{dict.headingQuiet}</span>
          </h1>

          <p className="text-fg-2 mx-auto mb-9 max-w-[46ch] text-[16.5px] leading-[1.55] text-pretty max-[520px]:text-[15.5px]">
            {dict.lead}
          </p>

          <div className="mb-6.5 grid w-full grid-cols-3 gap-3 max-[520px]:grid-cols-1">
            {destinations.map((dest) => (
              <Link
                key={dest.href}
                href={dest.href}
                className="group border-line bg-bg-card hover:border-line-2 hover:bg-bg-hover flex flex-col items-start gap-2.5 rounded-(--r-12) border p-[18px] text-left text-inherit no-underline transition-[border-color,background,transform] duration-(--d-base) ease-(--ease) hover:-translate-y-0.5 max-[520px]:flex-row max-[520px]:items-center max-[520px]:gap-3.5 max-[520px]:p-4"
              >
      ...
      <Footer dict={landingDict.footer} lang={lang} />
    </>
  )
}
