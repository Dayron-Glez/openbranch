...
export const StoryPanel = ({ dict, counts, challengeMinutes }: StoryPanelProps): ReactElement => {
  ...
  return (
    <aside className="bg-bg-elev border-line flex w-full flex-col justify-center gap-[30px] overflow-hidden rounded-(--r-16) border p-12 max-[1100px]:p-8 max-[760px]:gap-5 max-[760px]:p-6">
      <div className="flex flex-col gap-3">
        <span className="text-fg-muted font-mono text-[11px] tracking-[0.08em] uppercase">
          {dict.storyEyebrow}
        </span>
        <h2 className="text-fg m-0 max-w-[520px] text-[30px] leading-[1.2] font-light tracking-[-0.015em] text-pretty max-[1100px]:text-[25px] max-[760px]:text-[19px]">
          {dict.storyTitle} <span className="text-fg-2">{dict.storyTitleAccent}</span>
        </h2>
        <p className="text-fg-2 m-0 max-w-[500px] text-[14.5px] leading-[1.6] text-pretty max-[760px]:text-[13.5px]">
          {dict.storyLead}
        </p>
      </div>
      ...
      {stats.length > 0 && (
        <div className="flex items-center gap-7 pt-1 max-[760px]:gap-5">
